...
public class QuickSearchScreen extends Screen
{
    ...
    /// When the focus-hotbar key (a printable char, e.g. backtick) is consumed in keyPressed,
    /// GLFW still sends the matching charTyped right after. This flag swallows that one char
    /// event so the key isn't typed into the input widget.
    private boolean suppressNextCharInput = false;
    ...
    public void onClose()
    {
        if (QuickSearchClient.getConfig().search.rememberLastQuery) { lastQuery = this.inputWidget.getText(); }
        else { lastQuery = ""; }
        super.onClose();
    }
    ...
    protected void init()
    {
        ...
        if (lastQuery.equals("/") || lastQuery.isEmpty()) lastQuery = ""; // empty or just "/" doesn't count as a query
    ...
    {
        // Clear any stale suppression left by a previous (non-printable) focus-key press
        this.suppressNextCharInput = false;
                ...
                this.hotbarCollectionWidget != null && this.hotbarCollectionWidget.visible)
        {
            ...
            // the focus key is itself a printable char (e.g. backtick); swallow the
            // charTyped GLFW sends next so it isn't inserted into the input widget
            this.suppressNextCharInput = true;
            return true;
        }
    ...
    {
        // Eat the char event generated by the focus-hotbar key (see suppressNextCharInput)
        if (this.suppressNextCharInput)
        {
            this.suppressNextCharInput = false;
            return true;
        }

        return super.charTyped(chEv);
    }
